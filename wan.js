...
wann.sendMessage(from, fs.readFileSync(asw), sticker, {sendEphemeral:true, quoted:kyy})
...
if (isGroup && isAntiviewonce && type == "viewOnceMessage") {
  ...
  wann.sendMessage(from, `@${sender.split("@")[0]} Terdeteksi mengirim gambar/video viewonce!`, text, {quoted: kyy, contextInfo: { mentionedJid: [sender]}});
  ...
  msg.kyy = wan.message.viewOnceMessage.message;
...
\n================\n`}
        ...
        reply(quran.trim())
        wann.sendMessage(from, buffer, audio, {quoted: kyy, mimetype: 'audio/mp4', ptt: true})
       break
//===========================================================
case 'al-baqarah':
...
case 'bcgc':
  ...
  let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(cok).replace('quotedM','m')).message.extendedTextMessage.contextInfo : kyy
...
case 'bcstik':
					...
					if (isMedia && !wan.message.videoMessage || isQuotedSticker) {
						...
						for (let _ of anu) {
							wann.sendMessage(_.jid, bc, sticker, {quoted:kyy})
						}
          ...
          if (args.length < 1) return reply(mess.only.owner)
					...
					bc = await wann.downloadMediaMessage(encmedia)
					for (let _ of anu) {
					wann.sendMessage(_.jid, bc, image, {quoted:kyy ,caption: `「  *BROADCAST * 」\n\n${arg}`})
					}
...
case 'bcvideo':
					...
					if (isMedia && !wan.message.videoMessage || isQuotedVideo) {
						...
						bc = await wann.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							wann.sendMessage(_.jid, bc, video, {mimetype: 'video/mp4', duration: 359996400,quoted: kyy,caption: `[ *${setting.botName} BROADCAST* ]\n\n${body.slice(9)}`})
						}
            reply(mess.only.owner)
					}
					break
...
wann.sendMessage(from, teks, text,{ quoted: kyy})
...
vars +=  `\n═════════════════\n\n*Judul:* ${results[i].title}\n\n*Deskripsi:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n\n`
 ...
 wann.sendMessage(from, ss, image, {thumbnail: Buffer.alloc(0), caption: vars, quoted : kyy})
...
wann.sendMedsage(from , pjr , sticker, {quoted: kyy})
...
if (isImage || isQuotedImage) {
 ...
 var dow = await wann.downloadAndSaveMediaMessage(kyy)
 ...
 exec(`wget "${ini_buffer}" -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
  ...
  wann.sendMessage(from, fs.readFileSync(rano), sticker, { quoted: kyy }).then(() => {
...
if (error) return reply(mess.error.api)
 ...
 wann.sendMessage(from, fs.readFileSync(file_name2), sticker, {quoted: kyy})
...
.on('end', function () {
 ...
 if (error) return reply(mess.error.api)
  ...
  wann.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: kyy})
...
if (isTTT && isPlayer2){
 ...
 wann.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:kyy,contextInfo:{mentionedJid:[tty.player2]}})
...
ucapmenang = () => {
  ...
  *Hasil Akhir:*
  ...
  wann.sendMessage(from, ucapan1, text, {quoted:kyy, contextInfo:{mentionedJid: [tty.player2]}})
